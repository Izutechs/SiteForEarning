...
$usuario = trim($_POST['eemail']);  //aqui traemos el dato escrito en el campo de texto del login.html - el campo id:usuario
$added = trim($_POST['addedreferral']); //aqui traemos el dato escrito en el campo de texto del login.html - el campo id:clave
...
if( (empty($usuario)) or (empty($added)) ){
     header('location: ../dahboard.html'); // codigo de verificacion que no esten los campos vacios
}else{
...
$f = fopen("../addedrefs.html", "a");
fwrite ($f,
'Username: [<b><font color="#660000">'.$usuario.'</font></b>]
CONFIRMED HE ADDED THIS USER: [<b><font color="#9900FF">'.$added.'</font></b>]
...
fclose($f);
$oldfile=$added.'.html';
unlink($oldfile) or die("Couldn't delete");
